...
import {
  getAllReviews,
  removeAllReviews,
} from "../../redux/actions/getAllReviews";
...
export default function Review({ product }) {
  ...
  useEffect(() => {
    ...
    getUserById(currentUser?._id); // eslint-disable-next-line
  }, [currentUser]);
  ...
  return (
    <Box className={classes.reviewContainer}>
      ...
      <Grid container component="main" direction="column" alignItems="center">
        <Box component="fieldset" mb={3} className={classes.boxContainer}>
          ...
          >
            {suma / cant >= 0 && (
              <Typography variant="h6" style={{ marginRight: "1vw" }}>
                {(suma / cant).toFixed(1)}
              </Typography>
            )}
          ...
          {suma / cant >= 0 && (
            <Typography align="center">
              Promedio de {cant} {cant > 1 ? "opiniones" : "opinión"}
            </Typography>
          )}
          {cant < 1 && (
            <Typography align="center">
        ...
        {((flag && repeat === false) || edit === true) && (
          ...
          >
            <Typography component="legend">
              Califica el producto que compraste
            </Typography>
            <Grid
              container
            ...
            <Typography align="center" component="legend">
              Dejanos tu puntuación y un breve comentario
            </Typography>
            {!edit ? (
              <Button
                variant="contained"
                color="primary"
                disabled={!opinion || !punctuation}
          ...
          reviews?.map((review) => {
            return review?.product?._id === product?._id ? (
              ...
              >
                ...
                <Typography component="legend" style={{ marginTop: "3vh" }}>
                  {review.opinion}
                </Typography>
                  ...
                  {user && currentUser?._id === review?.user?._id && (
                    <>
                      <Button
                        ...
                        onClick={() => handleDelete(review._id, reviews.length)}
                      >
                        Eliminar
                      </Button>
                      <Button
